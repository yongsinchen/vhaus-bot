-- ══════════════════════════════════════════════════════════════════
-- Migration 098: apply_active_do_amendment() — explicit superseded_at guard,
-- layered on top of the CURRENT canonical definition (migration 097).
--
-- P1-3 stabilization. This is the same fix migration 095 drafted (never
-- applied — P1-3 was paused before it could be), but 095 was written against
-- migration 091's body. Migration 097 has since REPLACED 091 in production
-- (URGENT fix: removed the arrival-evidence gate entirely, and fixed a
-- second bug — a genuinely new proposed item never got a delivery_order_items
-- row on the replacement DO). Applying 095 now would silently overwrite BOTH
-- 097 fixes with the old, arrival-gated, new-item-dropping behavior — so 095
-- is retired unapplied and this migration reproduces 097's body VERBATIM
-- instead, with exactly one addition: `AND dord.superseded_at IS NULL` on
-- the affected-DO query.
--
-- WHY THIS GUARD IS NEEDED (found in the P1-3 lifecycle audit, empirically
-- re-confirmed against the live (097) function immediately before writing
-- this file — scripts/test-p1-3-amendment-superseded-guard.js fails 2/3
-- assertions against 097 exactly as it did against 091/095's predecessor).
-- The affected-DO query excluded a superseded DO only IMPLICITLY: a
-- superseded DO's items are assumed to no longer identity-match current
-- sales_order_items, since a normal supersession always carries them to a
-- replacement. That assumption doesn't hold for a DO manually marked
-- superseded (or any edge case) whose item still identity-matches a live
-- sales_order_item — the live RPC then re-supersedes it a second time,
-- confirmed via the test above. Every OTHER superseded_at-aware call site in
-- this codebase (this RPC's own supersede-eligibility partition, the
-- P1-1/P1-2/P1-3 scheduling guards, doLib.isOperationallyActive) checks
-- superseded_at directly rather than relying on this implicit side effect —
-- this migration closes the one remaining implicit case.
--
-- SCOPE: this migration changes ONLY the WHERE clause of the affected-DO
-- SELECT — everything else is reproduced verbatim from migration 097's
-- current body, preserving ALL of that migration's fixes:
--   - arrival status has zero gating effect on amendment approval
--     (no arrival-evidence conflict block, ever)
--   - a genuinely new proposed item (no source_item_id) gets a
--     delivery_order_items row on every replacement DO (v_new_item_ids)
--   - all other conflict checks (already_decided, stale_state,
--     active_do_in_transit, below_delivered_qty, removed_item_has_delivery)
--   - item upsert/delete, DO supersession + replacement + schedule
--     carry-forward, legacy projection write, final status flips
--
...
--   node scripts/test-p1-3-amendment-superseded-guard.js  -- must now pass 3/3.
--   node scripts/test-urgent-amendment-arrival-not-gating.js -- must still pass 24/24 (097's fixes untouched).
--   node scripts/test-p1-1-active-do-amendment.js -- must still pass in full.
--
-- Rollback: re-apply migration 097's body verbatim (or DROP FUNCTION and
-- reapply 097) — no data migration here, function logic only.
--
-- Migration 095 (091-based) is superseded by this file and must NOT be
-- applied — applying it after this migration (or after 097) would regress
-- both the arrival-gate removal and the new-item replacement-DO fix.
-- ══════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- P1-3 HARDENING (this migration): explicit `dord.superseded_at IS NULL`
  -- added — previously relied only on the implicit fact that a superseded
  -- DO's items no longer identity-match current sales_order_items. See
  -- header comment.
  ...
  IF v_supersede_do_ids IS NOT NULL THEN
    FOREACH v_old_do_id IN ARRAY v_supersede_do_ids LOOP
      ...
      -- Genuinely NEW proposed items (no source_item_id — never existed on
      -- the old DO) get a delivery_order_items row on the replacement DO too
      -- (migration 097's second fix — preserved verbatim here).
