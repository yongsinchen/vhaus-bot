-- ══════════════════════════════════════════════════════════════════
-- 062: Add service_type 4 = "Delivery" (deliver a missing item).
--
-- Adds a fourth service case type for delivering an item that was missing
-- from the original order. It needs one customer-facing delivery leg
-- (Warehouse → Customer), so the RPC gets an explicit branch instead of
-- falling into the assembly ELSE (whose leg is labelled "assembly visit").
--
--   Delivery (service_type 4) — 1 leg:
--     1. Warehouse → Customer   (deliver missing item)
--
-- Everything else in create_service_case() is unchanged from migration 029.
...
--
-- Rollback: re-run migrations/029_service_legs_by_type.sql
-- ══════════════════════════════════════════════════════════════════
