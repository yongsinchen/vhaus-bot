-- ══════════════════════════════════════════════════════════════════
-- 069: Add service_type 5 = "Delivery" (a plain delivery visit).
--
-- Adds a fifth service case type for a straightforward delivery to the
-- customer (distinct from type 4 "Delivery (Missing Item)"). It needs one
-- customer-facing delivery leg (Warehouse → Customer), so the RPC gets an
-- explicit branch instead of falling into the assembly ELSE (whose leg is
-- labelled "assembly visit").
--
--   Delivery (service_type 5) — 1 leg:
--     1. Warehouse → Customer   (deliver to customer)
--
-- Everything else in create_service_case() is unchanged from migration 062.
-- service_type has no CHECK constraint, so no column change is needed; only
-- the leg-generation branch is added. Existing cases are untouched.
--
-- Rollback: re-run migrations/062_service_type_delivery.sql
-- ══════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  ELSIF p_service_type = 4 THEN
    -- Delivery of a missing item: single delivery leg
    INSERT INTO service_legs (service_id, leg_order, leg_type, from_location, to_location, status, scheduled_date, notes)
    VALUES (v_service.id, 1, 'delivery', 'Warehouse', 'Customer', 'pending', v_date_d, 'Deliver missing item to customer');
  ELSIF p_service_type = 5 THEN
    -- Plain delivery: single delivery leg
    INSERT INTO service_legs (service_id, leg_order, leg_type, from_location, to_location, status, scheduled_date, notes)
    VALUES (v_service.id, 1, 'delivery', 'Warehouse', 'Customer', 'pending', v_date_d, 'Deliver to customer');
  ELSE
