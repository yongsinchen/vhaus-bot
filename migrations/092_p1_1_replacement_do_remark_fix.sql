-- ══════════════════════════════════════════════════════════════════
-- Migration 092: apply_active_do_amendment() — replacement DO loses remark
--
-- P0 HOTFIX (live data confirmed): SO 56190's delivery-instructions remark
-- ("Help to bring old sofa to downstairs skip tank, cost sgd 40-60 pay to
-- driver") is correctly on sales_orders.remark and correctly projected onto
-- the legacy orders row, but its only delivery_orders row (DO2609-0139) has
-- remark = NULL. Root cause #1 (fixed separately, application layer):
-- POST /sales-orders/:id/delivery-orders never defaulted an omitted/blank
-- request-body remark to the canonical sales_orders.remark. Root cause #2
-- (fixed here): this RPC's supersede-and-regenerate loop, when a P1-1
-- amendment approval regenerates a replacement delivery_orders row for an
-- affected draft/scheduled DO, does not carry `remark` into the new row's
-- INSERT INTO delivery_orders (...) column list AT ALL — so a replacement
-- DO silently drops whatever remark the superseded DO had, defaulting to
-- the column's NULL, regardless of what the SO's remark says.
--
-- FIX: add `remark` to the replacement DO's INSERT column/value list,
-- sourced from v_so_updated.remark — the POST-amendment canonical remark
-- (not the stale superseded DO's own v_do.remark), because the amendment
-- being approved may itself have changed sales_orders.remark (the amendment
-- flow can carry a header change alongside/instead of an item change), and
-- the regenerated replacement DO must reflect the current, applicable
-- delivery instruction, not a stale pre-amendment value. v_so_updated is
-- already computed earlier in this function (line ~370, before the
-- supersede loop) as v_so overlaid with the amendment's proposed header
-- fields, so it already carries the correct post-amendment remark whether
-- or not this particular amendment touched remark.
--
-- SCOPE: this migration ONLY adds `remark` to one INSERT statement's column
-- list and value list (the replacement-DO insert inside the supersede loop).
-- No other logic, signature, validation, or ordering changes. The function
-- body below is otherwise reproduced verbatim from migration 091 (the
-- currently-live version of apply_active_do_amendment — 091 supersedes 089
-- for this function; 090 is a different function, complete_delivery_order,
...
--   node scripts/test-p0-do-remark.js  -- covers replacement-DO remark carry-forward (test #4).
--
-- Rollback: re-apply migration 091's function body verbatim (drops the
-- remark column back out of the replacement-DO insert). No data migration
-- here — this only changes function logic going forward; it does not
-- backfill any existing replacement DO's remark.
-- ══════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- using it for the step-8 legacy-evidence containment check.
  ...
  IF v_supersede_do_ids IS NOT NULL THEN
    FOREACH v_old_do_id IN ARRAY v_supersede_do_ids LOOP
      ...
      -- FIX (this migration): carry the SO's delivery remark/instructions
      -- forward onto the regenerated replacement DO. Sourced from
      -- v_so_updated.remark (post-amendment canonical value), not
      -- v_do.remark (the superseded DO's own, possibly-stale remark) —
      -- see header comment.
  ...
  UPDATE orders SET
    ...
    -- replicate the existing double-encoding convention (to_jsonb of the
    -- text form) instead of an invalid direct text->jsonb assignment. See
    -- migration 091's header comment.
