...
function createDeliveryDateApprovalService({ supabase, isLockedScheduleStatus, logDoEvent }) {
  ...
  async function applyApprovedDeliveryDate(reqRow, actorId = null) {
    ...
    if (reqRow.order_id) {
      const { data: svc } = await supabase.from("services").select("id").eq("legacy_order_id", reqRow.order_id).maybeSingle();
      if (svc) {
        await supabase.from("services").update({ due_date: newDate }).eq("id", svc.id);
