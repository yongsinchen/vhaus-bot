...
const { serviceStatusAfterDateChange } = require("./service-lifecycle");
 ...
 *
 * URGENT FIX (post-P1-4D): approval is required when EITHER the requested
 * date OR the CURRENT/original operational date falls inside the protected
 * 10-day window — not the requested date alone. Otherwise a salesman could
 * escape the approval requirement simply by moving an already-near delivery
 * to a date far in the future (current=D+4, requested=D+30 used to
 * auto-approve; it must not). Both sides use the exact same inclusive-
 * boundary comparison as before (`< thresholdDate`), so the D+10 boundary
 * itself is unchanged — see scripts/test-p1-2-delivery-date-approval.js's
 * existing boundary assertions, still authoritative and unmodified here.
 *
 * @param {object} p
 * @param {string} p.requestedDate - YYYY-MM-DD
 * @param {string|null} [p.currentDate] - YYYY-MM-DD, the operational date
 *   BEFORE this change (original_date). Optional/nullable — a brand-new
 *   order with no prior operational date has nothing to protect on that
 *   side, so omitting it (or passing null/invalid) behaves exactly like
 *   the pre-fix requested-date-only rule.
 ...
 * @returns {{valid:boolean, requestedDate:string|null, currentDate:string|null,
 *   thresholdDate:string|null, requiresApproval:boolean|null, autoApproved:boolean,
 *   requestedDateWithinWindow:boolean, currentDateWithinWindow:boolean, reason:string}}
 */
function evaluateDeliveryDateApproval({ requestedDate, currentDate, today } = {}) {
  ...
  const cleanCurrentDate = DATE_RE.test(currentDate) ? currentDate : null;

  if (!requestedDate || !DATE_RE.test(requestedDate)) {
    return { valid: false, requestedDate: requestedDate || null, currentDate: cleanCurrentDate, thresholdDate, requiresApproval: null, autoApproved: false, requestedDateWithinWindow: null, currentDateWithinWindow: null, reason: "invalid_format" };
  }
  if (requestedDate < todayStr) {
    ...
    return { valid: false, requestedDate, currentDate: cleanCurrentDate, thresholdDate, requiresApproval: null, autoApproved: false, requestedDateWithinWindow: null, currentDateWithinWindow: null, reason: "past_date" };
  }

  // "Within the protected window" = strictly before the threshold — the
  // SAME boundary the old requested-date-only check used (D+10 itself is
  // NOT protected, matching the existing, unmodified boundary semantics).
  // A current date already in the past (overdue) still counts as within
  // the window — it is certainly not "safely far in the future" and
  // deserves at least as much protection as a near-term date, not less.
  const requestedDateWithinWindow = requestedDate < thresholdDate;
  const currentDateWithinWindow = cleanCurrentDate != null && cleanCurrentDate < thresholdDate;
  const requiresApproval = requestedDateWithinWindow || currentDateWithinWindow;
  const autoApproved = !requiresApproval;

  let reason;
  if (autoApproved) reason = "auto_approved_threshold_met";
  else if (currentDateWithinWindow && requestedDateWithinWindow) reason = "pending_admin_approval_both_dates_within_window";
  else if (currentDateWithinWindow) reason = "pending_admin_approval_current_date_within_window";
  else reason = "pending_admin_approval_requested_date_within_window";

  return {
    valid: true,
    requestedDate,
    currentDate: cleanCurrentDate,
    thresholdDate,
    requiresApproval,
    autoApproved,
    requestedDateWithinWindow,
    currentDateWithinWindow,
    reason,
  };
}
...
function createDeliveryDateApprovalService({ supabase, isLockedScheduleStatus, logDoEvent }) {
  ...
  async function applyApprovedDeliveryDate(reqRow, actorId = null) {
    ...
    } else if (reqRow.so_number && reqRow.company_id) {
      // P1-4B: fail closed — so_number alone is not a safe identity for a
      // WRITE. Without reqRow.company_id this would update every company's
      // order sharing this so_number; skip the write entirely rather than
      // risk that (reqRow.company_id is set at request-creation time and
      // should always be present — this is defense in depth, not the
      // expected path).
      await supabase.from("orders").update({ delivery_date: newDate }).eq("company_id", reqRow.company_id).eq("so_number", reqRow.so_number);
    }
    ...
    // P1-5: a request originated by a Service Case (requested_via:
    // "service_case") targets order_id with no DO — the whole-order team
    // resync above already re-homes delivery_schedules, but services.due_date
    // and service_legs.scheduled_date (which drive the Services list and the
    // printed service note, per PATCH /service-cases/:id) are a separate
    // representation this function otherwise has no way to know about.
    // Without this, an approved Service-date change would move
    // orders.delivery_date/delivery_schedules but leave the Services list and
    // printed note on the stale date.
    if (reqRow.order_id) {
      const { data: svc } = await supabase.from("services").select("id, status").eq("legacy_order_id", reqRow.order_id).maybeSingle();
      if (svc) {
        // Approving a date request APPLIES a real operational date, so the same
        // open -> scheduled lifecycle transition the direct-apply path performs
        // must happen here too — otherwise a Service scheduled via approval
        // keeps status 'open'. Centralized in lib/service-lifecycle.js; every
        // non-open state (in_progress/resolved/…) is preserved.
        const svcUpdate = { due_date: newDate };
        const nextStatus = serviceStatusAfterDateChange(svc.status, true);
        if (nextStatus !== svc.status) svcUpdate.status = nextStatus;
        await supabase.from("services").update(svcUpdate).eq("id", svc.id);
        await supabase.from("service_legs").update({ scheduled_date: newDate })
          .eq("service_id", svc.id).not("status", "in", "(completed,cancelled)");
      }
    }
    return result;
  }
