...
// linked_so + service_note (bug 4): for Service-type stops the board shows the
// linked SO and the service description in their own fields instead of dumping
// the combined note into the generic Remark strip.
// P0 hotfix: delivery_orders.remark was missing from this nested select —
// DRIVER_SCHEDULE_SELECT below already had it, this one didn't. StopRow
// (on-screen assigned stop) and TeamPrintView (Print Schedule) both read
// sc.delivery_orders.remark for a DO-based stop; without it here that read
// was always undefined for both, regardless of how correct their own
// rendering code was. Root-caused by reproducing the board's exact query
// live and inspecting the returned delivery_orders keys.
// P1-1 stabilization: superseded_at/superseded_by_do_id added — StopRow (the
// assigned-stop card on this same board) had no way to detect a superseded
// DO because this select never carried the fields at all, on top of never
// checking them. superseded_by's do_number is joined the same way
// DELIVERY_ORDER_LIST_SELECT already does, so a superseded stop can render
// "Superseded → see DOxxxx" instead of looking like a normal live delivery.
const DELIVERY_SCHEDULE_LIST_SELECT = `*,
  orders(so_number, customer_name, address, contact, items, status, order_amount, balance, type, salesman, time_slot, remark, linked_so, service_note),
  delivery_teams(vehicle_id, driver_id, delivery_vehicles(vehicle_plate)),
  delivery_orders(id, do_number, status, remark, superseded_at, superseded_by_do_id,
    superseded_by:delivery_orders!superseded_by_do_id(do_number),
    delivery_order_items(product_code, product_name, supplier_name, size, color, quantity, status))`;
...
// Delivery Orders tab). supplier_name (fix #1) added so the pool card can
// show it without a second request. delivery_schedules/delivery_teams join
// added so the Delivery Orders tab can show which team a scheduled DO is
// assigned to — a DO carries no team_id of its own (see migrations/015's
// comment: driver/team/vehicle live on the delivery_schedules attempt
// layer, not the DO), so it has to come in through the schedule row. A DO
// can have more than one schedules row (e.g. a failed attempt followed by
// a reschedule) — consumers should pick the latest/non-terminal one.
// P1-1: superseded_at/superseded_by_do_id/supersedes_do_id (migration 085)
// already ride along in the leading "*" once applied — the two joins below
// add the OTHER side's do_number so the frontend can render a "Superseded —
// see DO<x>" label without a second request. Self-referencing FK aliases
// follow this file's existing driver:users!<fkey>(...) convention.
const DELIVERY_ORDER_LIST_SELECT = `*,
  ...
  sales_orders(id, order_number, customer_name, customer_contact, customer_address, salesman_name, delivery_date, delivery_time_slot,
    sales_order_items(product_code, product_name, arrived_at)),
  delivery_schedules(id, status, team_id, scheduled_date,
    delivery_teams(vehicle_id, driver_id, delivery_vehicles(vehicle_plate), driver:users!delivery_teams_driver_id_fkey(name))),
  superseded_by:delivery_orders!superseded_by_do_id(do_number),
...
// `salesman` is the raw "A / B" list off the order. The Commission payout view
// needs it to show each person their SHARE of a split order: commission_amt is
// already divided by the number of names (calculateCommission), but net_amount
// is the whole order, so the UI has to divide it by the same count to display a
// sales figure that reconciles with the payout.
const COMMISSION_LIST_SELECT = `*,
  orders(so_number, customer_name, order_amount, balance, status, company_id, order_date, salesman),
