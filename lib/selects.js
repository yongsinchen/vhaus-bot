...
const DELIVERY_SCHEDULE_LIST_SELECT = `*,
  orders(so_number, customer_name, address, contact, items, status, balance, type, salesman, time_slot, remark, linked_so, service_note),
...
// order_date is read by the Commission page's order-details modal and is the basis
// for month-scoping pending commissions (see COMMISSION_LIST_SELECT_ORDER_INNER).
const COMMISSION_LIST_SELECT = `*,
  orders(so_number, customer_name, order_amount, balance, status, company_id, order_date),
  users(name, salesman_name)`;

// Same shape, but an INNER join on orders so the caller can filter by a column on
// the joined order (e.g. .gte("orders.order_date", …)). PostgREST only applies a
// nested filter as a real restriction when the embed is !inner; with the default
// left join the parent row survives with orders: null. Used to scope pending
// commissions — which carry no payout_month — to a payout batch by order date.
// Note: commissions whose order has a NULL order_date are excluded by such a
// filter, so keep it to queries where that is the intended behaviour.
const COMMISSION_LIST_SELECT_ORDER_INNER = COMMISSION_LIST_SELECT.replace("orders(", "orders!inner(");
...
module.exports = {
  ...
  COMMISSION_LIST_SELECT_ORDER_INNER,
};
