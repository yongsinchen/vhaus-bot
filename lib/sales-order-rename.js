...
async function renameSalesOrderNumber(supabase, { id, companyId, newNumber, actor }) {
  ...
  // P1-5: a Service Case's synthetic order row stores the source SO's number
  // as free text (orders.linked_so, copied once at case-creation time) — this
  // rename audit (see file header) covers every table with a copied
  // so_number EXCEPT this one, which was missed. Best-effort, non-fatal:
  // this is display-only staleness (the "Linked to SO: X" label), never a
  // reason to block or fail the rename itself. Scoped to type='Service' so
  // this can only ever touch a Service Case's own synthetic order.
  try {
    await supabase.from("orders").update({ linked_so: trimmed })
      .eq("company_id", companyId).eq("linked_so", oldNumber).eq("type", "Service");
  } catch (e) { console.error("linked_so resync (order-number rename, non-fatal):", e.message); }
