    ...
    for (const item of items) {
      ...
      if (item._target && item._target.order_id != null && item._target.item_index != null) {
        ...
        if (!order || !ownershipOk || !target) {
          ...
          oItems[item._target.item_index] = applyArrivalToItem(target, parseQty(item.quantity), arrivalDate);
      ...
      for (const order of orders) {
        ...
        if (hitIdx >= 0) {
          oItems[hitIdx] = applyArrivalToItem(oItems[hitIdx], parseQty(item.quantity), arrivalDate);
          await supabase.from("orders").update({ items: JSON.stringify(oItems) }).eq("id", order.id);
          await syncArrivalsToSalesOrderItems(order.id);
