...
// Union matcher: true when a DO line refers to this order-JSON item.
function itemMatchesOrderItem(doItem, doKeywords, oi) {
  ...
  // 4. Keyword match: >= 2 keywords, or one keyword >= 5 chars
...
function createSupplierDOService(deps) {
  ...
  async function findCandidateOrders(soNumber, companyId) {
    let q = supabase.from("orders")
      .select("id, so_number, items, status, company_id, customer_name")
      .eq("so_number", soNumber).in("status", ["Pending", "In Progress"]);
    ...
    for (const item of items || []) {
      ...
      for (const order of orders) {
        ...
        oItems.forEach((oi, idx) => {
          const hit = itemMatchesOrderItem(item, doKeywords, oi);
          row.candidates.push({
    ...
    for (const item of items) {
      ...
      for (const order of orders) {
        ...
        const updatedItems = oItems.map(oi => {
          if (!itemMatchesOrderItem(item, doKeywords, oi)) return oi;
  ...
  return { previewMatch, processSupplierDOUpload, storeDOPhoto, findProductMasterMatch };
}
