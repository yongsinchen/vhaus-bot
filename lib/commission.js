...
// Inverse of getPayoutMonth (server.js): the range of order dates that feed a given
// payout month. Payout month M is fed by orders dated in month M-1, so payout
// 2026-09-01 covers 2026-08-01 … 2026-08-31. Built in UTC so the boundary can't
// drift a day either way depending on where the process runs.
//
// Eligible/held/paid commissions carry payout_month and are filtered on it directly.
// This exists for PENDING commissions, whose payout_month stays NULL until the
// deposit gate is met, and which therefore have to be bucketed into a payout batch
// by their order's own date instead.
function getOrderMonthRange(payoutMonth) {
  const [y, m] = String(payoutMonth).slice(0, 7).split("-").map(Number);
  if (!y || !m) return null;
  return {
