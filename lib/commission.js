...
// ── Product incentives ───────────────────────────────────────────────────
//
// Match an order's items against the company's product incentives and return one
// row per earning item. Extracted from calculateCommission so the SAME matching
// rule serves both the calculation and GET /orders/:id/incentive-items — the UI
// must offer exactly the incentives the money math will use, not a second guess.
//
// Matching is EXACT on product_id — the canonical variant identity
// (company_id + exact sales_order_item.product_id = product_incentives.product_id).
// There is NO fuzzy fallback: product_name / product_code / substring / LIKE
// matching was removed because a model-level name (e.g. "ALESSIO", identical
// across Queen/King/Super Single) made every variant inherit another variant's
// incentive and let an arbitrary first-match win. Rules:
//   • item has no resolvable product_id            → RM0 (no incentive)
//   • no active config for that product_id         → RM0
//   • exactly one active config                    → use its incentive_amount
//   • more than one active config for the SAME
//     product_id (a configuration conflict)        → RM0, flagged (never
//                                                     first-match / sum / guess)
// `incentives` MUST already be pre-filtered to active + in-date (the caller's
// commission cache does this), and each `item` MUST carry a resolved
// `product_id` (enriched from its soiId → sales_order_items.product_id).
//
// `excludedIds` are product_incentives ids a manager has switched off for this
// order. They are keyed on the incentive, not the item, because order items carry
// no stable id — a positional key would silently reattach to the wrong item the
// moment the order is edited.
function matchProductIncentives(orderItems, incentives, excludedIds = []) {
  const excluded = new Set((excludedIds || []).map(String));
  const items = Array.isArray(orderItems) ? orderItems : [];
  const active = incentives || [];
  const rows = [];
  for (const item of items) {
    const pid = item.product_id != null && item.product_id !== "" ? String(item.product_id) : null;
    const qty = Number(item.unit) || 1;
    if (!pid) continue; // no exact product identity → no incentive (fail closed)
    const matches = active.filter(inc => inc.product_id != null && String(inc.product_id) === pid);
    if (matches.length === 0) continue; // no exact config → RM0
    if (matches.length > 1) {
      // Configuration conflict: multiple active configs for one product_id.
      // Fail closed — contribute RM0 and surface the conflict for admin review;
      // never pick first, never sum.
      rows.push({
        incentive_id: null, product_id: pid,
        product_name: item.itemName || "Item", product_code: item.itemCode || null,
        item_name: item.itemName || null, qty, unit_amount: 0, amount: 0,
        excluded: true, conflict: true, conflict_incentive_ids: matches.map(m => m.id),
      });
      continue;
    }
    const match = matches[0];
    const unitAmount = Number(match.incentive_amount) || 0;
    rows.push({
      incentive_id: match.id,
      product_id: pid,
      product_name: match.product_name || match.product_code || item.itemName || "Item",
      product_code: match.product_code || null,
      item_name: item.itemName || null,
      qty,
      unit_amount: unitAmount,
      amount: round2(unitAmount * qty),
      excluded: excluded.has(String(match.id)),
    });
  }
  return rows;
}

// Order-level product incentive actually payable — excluded rows contribute nothing.
function payableProductIncentiveTotal(rows) {
  return round2((rows || []).reduce((s, r) => s + (r.excluded ? 0 : Number(r.amount) || 0), 0));
}

// ── Deterministic legacy-item → product_id fallback ──────────────────────
//
// The primary product-id resolution is the immutable soiId
// (soiId → sales_order_items.product_id). Older legacy `orders.items` rows
// predate soiId stamping and carry none, so they would fail closed to RM0 even
// when the sold product plainly has an incentive config. This is the ONLY
// fallback: match a legacy item to the order's sales_order_items on the SAME
// key the projection itself is built from — product_code + the composed name
// (product_name + size + color + custom_dimensions) — PLUS quantity, and
// resolve ONLY when exactly one distinct product_id matches. It is deliberately
// NOT fuzzy/substring and NOT positional:
//   • 0 candidates          → null (stay RM0, fail closed)
//   • exactly 1 product_id  → that product_id
//   • >1 distinct product_id→ null (ambiguous — never guess, never by position)
// Validated read-only against production before shipping: on 1018 items whose
// soiId already resolves (ground truth) the key reproduced the SAME product_id
// with 0 wrong and 0 ambiguous (the rest found no candidate and stayed RM0).
function normLegacyKeyPart(s) {
  return String(s == null ? "" : s).toLowerCase().replace(/\s+/g, " ").trim();
}
function legacyItemMatchKey(code, name, qty) {
  return `${normLegacyKeyPart(code)}|${normLegacyKeyPart(name)}|${Number(qty) || 1}`;
}
// Build key → Set(product_id) from an order's sales_order_items rows. Each row:
// { product_id, product_code, product_name, size, color, custom_dimensions, quantity }.
function buildDeterministicPidIndex(soiRows) {
  const byKey = new Map();
  for (const r of (soiRows || [])) {
    if (r == null || r.product_id == null) continue;
    const composedName = [r.product_name, r.size, r.color, r.custom_dimensions].filter(Boolean).join(" ");
    const k = legacyItemMatchKey(r.product_code, composedName, r.quantity);
    if (!byKey.has(k)) byKey.set(k, new Set());
    byKey.get(k).add(String(r.product_id));
  }
  return byKey;
}
// Resolve ONE legacy item ({ itemCode, itemName, unit }) against a prebuilt
// index. Returns a product_id string only on a unique match, else null.
function resolveLegacyItemProductId(item, pidIndex) {
  if (!item || !pidIndex) return null;
  const set = pidIndex.get(legacyItemMatchKey(item.itemCode, item.itemName, item.unit));
  return set && set.size === 1 ? [...set][0] : null;
}

module.exports = {
  // Product incentives
  matchProductIncentives,
  payableProductIncentiveTotal,
  // Deterministic legacy-item → product_id fallback (soiId is primary)
  legacyItemMatchKey,
  buildDeterministicPidIndex,
  resolveLegacyItemProductId,
  // SG
