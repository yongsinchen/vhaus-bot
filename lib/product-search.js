...
// Resolve a search into the list of `or=(...)` groups to apply — one per token,
// empty when there is nothing to search for. Supplier names have to be looked
// up in the database, so this step is async.
//
// It deliberately returns filter STRINGS rather than a query builder. An async
// function that returns a Supabase builder is a trap: the builder is thenable,
// so awaiting the returned promise adopts it, FIRES THE QUERY, and hands back a
// response object instead of a builder — every filter chained afterwards then
// throws "query.eq is not a function". Keep the async step data-only and let
// the caller apply the result synchronously.
async function buildProductSearchFilters({ supabase, companyId, search }) {
  const tokens = tokenizeSearch(search);
  if (tokens.length === 0) return [];

  const supplierMatches = await resolveSupplierMatches(supabase, companyId, tokens);
  return tokens.map(token =>
    buildTokenFilter(token, { supplierIds: supplierMatches.get(token) || [] }));
}

// Chain the resolved filter groups onto a query builder. Synchronous, so it is
// safe to return the builder. Repeated .or() calls are AND-combined by
// PostgREST, giving the all-tokens-must-match semantics.
function applyFilters(query, filters) {
  for (const filter of filters) query = query.or(filter);
  return query;
}

// Same token semantics for a table without a supplier_id column — used by the
// organization_products master typeahead. No supplier lookup, so no async step
// is needed and returning the builder is safe.
function applyColumnSearch(query, search, columns = PRODUCT_SEARCH_COLUMNS) {
  const tokens = tokenizeSearch(search);
  return applyFilters(query, tokens.map(token =>
    buildTokenFilter(token, { columns, supplierIds: [] })));
}
